...
layout: post
---
# M1 GPUs for (scientific) computations in C++: SAXPY and finite differences
...
void MetalAdder::sendComputeCommand()
{
    ...
    // Wrap our computation and data up into something our Metal device understands
    ...
    // Normally, you want to do other work in your app while the shader is running,
                                ...
                                size_t arrayLength)
{
    ...
    // You could put this somewhere else, if you want asynchronous computations
    commandBuffer->waitUntilCompleted();
}
```
...
```cpp
    ...
    std::cout << "x: " << x[index] << " y: " << y[index] << " r: " << r[index] << std::endl;
...
OpenMP (10 threads):      5248.47ms +/- 482.052ms  <-- 1.8x speed-up fastest OpenMP
...
OpenMP (8 threads):       9877.14ms +/- 746.775ms <-- 4.7x speed-up, fastest OpenMP
